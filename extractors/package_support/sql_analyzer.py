...
from extractors.package_support.oracle_parser import OracleSqlParser
...
def analyze_sql(text: str) -> SqlAnalysis:
    ...
    calls = [CallReference(ref.object_name, ref.start) for ref in parser.calls()]
    if not calls:
        calls = [CallReference(ref.object_name, ref.start) for ref in extract_sql_calls(text)]
    ...
    parse_errors = parser.parse_error_offsets() or [match.start() for match in _MALFORMED_RE.finditer(text)]
