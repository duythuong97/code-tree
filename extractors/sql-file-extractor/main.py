...
from contract.graph_contract import sql_file_id, table_id
...
from extractors.package_support.semantic_tree import attach_sql_semantic_tree
from extractors.package_support.sql_loader import extract_sql_loader
...
_CALL_RE = re.compile(r"\b([A-Za-z_][\w$#]*)\.([A-Za-z_][\w$#]*)\s*\(", re.IGNORECASE)
...
def extract(config: dict) -> None:
    ...
    for file in files:
        ...
        for call in _CALL_RE.finditer(file.text):
            package = call.group(1).upper()
            routine = call.group(2).upper()
            raw = f"{package}.{routine}"
