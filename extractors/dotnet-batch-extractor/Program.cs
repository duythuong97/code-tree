...
var methods = DiscoverMethods(csFiles);
...
{
    var model = compilation.GetSemanticModel(file.SyntaxTree!);
    invocations.AddRange(ExtractorRuntime.MethodInvocationEdges(file, model, classNames));
    invocations.AddRange(TopLevelInvocationEdges(file, model, classNames));
}
var commandHandlers = DiscoverCommandHandlers(csFiles, compilation, classNames);
var dataFindings = CollectDataFindings(csFiles, compilation, catalog, folderContexts, defaultDatabase);
...
foreach (var executable in executables.OrderBy(exe => exe.Name, StringComparer.OrdinalIgnoreCase))
{
    ...
    var reachable = ReachableMethods(roots, invocations);
    ...
    {
        var sourceKey = new MethodKey(invocation.SourceClass, invocation.SourceMember);
        var targetKey = new MethodKey(invocation.TargetClass, invocation.TargetMember);
    ...
    var rootMethod = roots.Select(key => methodLookup.GetValueOrDefault(key)).FirstOrDefault(method => method is not null);
    if (rootMethod?.File.SyntaxTree is not null)
    {
        var syntax = rootMethod.File.SyntaxTree.GetRoot().FindToken(rootMethod.Start).Parent?.AncestorsAndSelf().OfType<MethodDeclarationSyntax>().FirstOrDefault();
        var steps = invocations.Where(invocation => roots.Contains(new MethodKey(invocation.SourceClass, invocation.SourceMember)) && reachable.Contains(new MethodKey(invocation.TargetClass, invocation.TargetMember)))
            .Select(invocation => SemanticTreeV2.Fact("call", $"Call {invocation.TargetClass}.{invocation.TargetMember}", invocation.File, invocation.Start, NodeForMethod(new MethodKey(invocation.TargetClass, invocation.TargetMember), builder, nodeCache, methodLookup, repository, systemKey, executable.Database, folderContexts))).ToList();
        builder.SetNodeProperty(entryId, "semantic_tree", SemanticTreeV2.Operation("Main", syntax?.ParameterList.Parameters, steps, rootMethod.File, syntax));
    }
    foreach (var handler in handlers)
    {
        var modeId = ExtractorRuntime.StableNodeId("command-mode", executable.Key, ExtractorRuntime.Slug(handler.Mode));
        var handlerNode = NodeForMethod(handler.Handler, builder, nodeCache, methodLookup, repository, systemKey, executable.Database, folderContexts);
        var step = SemanticTreeV2.Fact("call", $"Call {handler.Handler.ClassName}.{handler.Handler.MemberName}", handler.File, handler.Start, handlerNode);
        builder.SetNodeProperty(modeId, "semantic_tree", SemanticTreeV2.Operation(handler.Mode, null, new[] { step }));
    }
}
...
static List<MethodDeclarationInfo> DiscoverMethods(List<SourceFile> csFiles)
{
    var methods = new List<MethodDeclarationInfo>();
    foreach (var file in csFiles)
    {
        var root = file.SyntaxTree!.GetRoot();
        var firstGlobalStatement = root.DescendantNodes().OfType<GlobalStatementSyntax>().FirstOrDefault();
        if (firstGlobalStatement is not null)
            methods.Add(new MethodDeclarationInfo(TopLevelKey(file), file, firstGlobalStatement.SpanStart, true));
        foreach (var cls in root.DescendantNodes().OfType<ClassDeclarationSyntax>())
        {
            foreach (var method in cls.Members.OfType<MethodDeclarationSyntax>())
            {
                var key = new MethodKey(cls.Identifier.Text, method.Identifier.Text);
                var isEntry = method.Identifier.Text.Equals("Main", StringComparison.Ordinal) && method.Modifiers.Any(SyntaxKind.StaticKeyword);
                methods.Add(new MethodDeclarationInfo(key, file, method.SpanStart, isEntry));
            }
        }
    }
    return methods;
...
static IEnumerable<MethodInvocationInfo> TopLevelInvocationEdges(SourceFile file, SemanticModel model, HashSet<string> classNames)
{
    ...
    foreach (var invocation in file.SyntaxTree.GetRoot().DescendantNodes().OfType<InvocationExpressionSyntax>())
    {
        ...
        yield return new MethodInvocationInfo(source.ClassName, source.MemberName, target.ClassName, target.MemberName, invocation.SpanStart, file);
    }
}

static List<CommandHandlerInfo> DiscoverCommandHandlers(List<SourceFile> csFiles, CSharpCompilation compilation, HashSet<string> classNames)
{
    ...
    foreach (var file in csFiles)
    {
        ...
        {
            var source = OwnerForNode(file, ifStatement);
            if (!IsValidOwner(source)) continue;
            var target = FirstHandlerTarget(ifStatement.Statement, model, classNames, source);
            if (target is null) continue;
            foreach (var mode in ModesFromCondition(ifStatement.Condition))
                handlers.Add(new CommandHandlerInfo(mode, source, target, file, ifStatement.SpanStart));
        }

        foreach (var switchStatement in root.DescendantNodes().OfType<SwitchStatementSyntax>())
        {
            var source = OwnerForNode(file, switchStatement);
            if (!IsValidOwner(source)) continue;
            foreach (var section in switchStatement.Sections)
            {
                var target = FirstHandlerTarget(section, model, classNames, source);
                if (target is null) continue;
                foreach (var mode in section.Labels.OfType<CaseSwitchLabelSyntax>().SelectMany(label => ModeValues(label.Value)))
                    handlers.Add(new CommandHandlerInfo(mode, source, target, file, section.SpanStart));
            }
        }

        foreach (var switchExpression in root.DescendantNodes().OfType<SwitchExpressionSyntax>())
        {
            var source = OwnerForNode(file, switchExpression);
            if (!IsValidOwner(source)) continue;
            foreach (var arm in switchExpression.Arms)
            {
                var target = FirstHandlerTarget(arm.Expression, model, classNames, source);
                if (target is null) continue;
                foreach (var mode in ModeValuesFromPattern(arm.Pattern))
                    handlers.Add(new CommandHandlerInfo(mode, source, target, file, arm.SpanStart));
            }
        }
    }
    return handlers;
...
static MethodKey? FirstHandlerTarget(SyntaxNode node, SemanticModel model, HashSet<string> classNames, MethodKey source)
{
...
static List<DataFinding> CollectDataFindings(List<SourceFile> csFiles, CSharpCompilation compilation, Catalog catalog, List<FolderContext> folderContexts, string defaultDatabase)
{
    ...
    foreach (var file in csFiles)
    {
        ...
        {
            var owner = OwnerForOffset(file, expression.Start);
...
static HashSet<MethodKey> ReachableMethods(HashSet<MethodKey> roots, List<MethodInvocationInfo> invocations)
{
    var reachable = new HashSet<MethodKey>(roots);
    var queue = new Queue<MethodKey>(roots);
    var outgoing = invocations
        .GroupBy(invocation => new MethodKey(invocation.SourceClass, invocation.SourceMember))
        .ToDictionary(group => group.Key, group => group.Select(invocation => new MethodKey(invocation.TargetClass, invocation.TargetMember)).Distinct().ToList());
    while (queue.Count > 0)
    {
...
static string NodeForMethod(MethodKey key, PackageBuilder builder, Dictionary<MethodKey, string> cache, Dictionary<MethodKey, MethodDeclarationInfo> methods, string repository, string systemKey, string fallbackDatabase, List<FolderContext> folderContexts)
{
    ...
    {
        var nodeId = ExtractorRuntime.StableNodeId("repository", repository, key.ClassName);
        builder.AddNode(nodeId, "REPOSITORY", key.ClassName, $"{repository}.{key.ClassName}", key.ClassName, systemKey, database, repository, "TECHNICAL");
        return cache[key] = nodeId;
    }
    if (key.ClassName.EndsWith("Service", StringComparison.Ordinal))
    {
        var nodeId = ExtractorRuntime.StableNodeId("service", repository, key.ClassName);
        builder.AddNode(nodeId, "SERVICE", key.ClassName, $"{repository}.{key.ClassName}", key.ClassName, systemKey, database, repository, "TECHNICAL");
        return cache[key] = nodeId;
    }
    var localId = ExtractorRuntime.StableNodeId("local-routine", repository, $"{key.ClassName}.{key.MemberName}");
    builder.AddNode(localId, "LOCAL_ROUTINE", key.MemberName, $"{repository}.{key.ClassName}.{key.MemberName}", key.MemberName, systemKey, database, repository, "TECHNICAL");
    return cache[key] = localId;
}
...
static MethodKey? ResolveInvocationTarget(InvocationExpressionSyntax invocation, SemanticModel model, HashSet<string> knownClasses)
{
    ...
    if (symbol is not null)
    {
        ...
        if (!string.IsNullOrEmpty(known)) return new MethodKey(known, symbol.Name);
    }
...
static MethodKey OwnerForNode(SourceFile file, SyntaxNode node)
{
    ...
    if (!string.IsNullOrEmpty(cls) && !string.IsNullOrEmpty(member)) return new MethodKey(cls, member);
...
static MethodKey TopLevelKey(SourceFile file)
    => new("$TopLevel", ExtractorRuntime.RepositoryPath(file.RelativePath));
...
sealed record MethodKey(string ClassName, string MemberName);
sealed record MethodDeclarationInfo(MethodKey Key, SourceFile File, int Start, bool IsEntryPoint);
sealed record CommandHandlerInfo(string Mode, MethodKey Source, MethodKey Handler, SourceFile File, int Start);
